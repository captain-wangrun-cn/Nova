...
## 六·补十一 · 第六轮 ①：int8 融合注意力核（✅ 2026-09-23 —— **ULP 级一致，16K 单层 7.53x**）

**报告：[reports/kv-int8-fused-attn.md](reports/kv-int8-fused-attn.md) · 决策 D42 · 代码 `src/nova/kvattn.py` ·
测试 `tests/test_kvattn.py`（21 条）· 基准 `src/diagnostics/bench_int8_attn.py` · 真数据复核 `probe_kvattn_real.py`**

靶子由 **D38 + D40** 定死（int8、K 按 token 维分组、每 64 token 一条尺子），**没有重新选型**。

| 项 | 结果 |
|---|---|
| 合成 K/V（10 长度 × 3 种子，覆盖分组/分块边界） | 全部 **≤ 1.00 ULP**，无元素 > 2 ULP |
| 真模型 K/V（1894 token，layer 0/18/35） | **1.00 / 2.00 / 31.00 ULP**；超 2 ULP 的 0/0/**6** 个元素（绝对偏差都 ≤ 3.9e-6） |
| 16K 单层解码（CUDA Graph） | **0.201 ms vs SDPA 1.513 ms = 7.53x**（带宽 156.7 GiB/s = 上限 67.4%） |
| 8192 单层 | 0.081 ms vs 0.761 ms = **9.33x**（193.2 GiB/s = **83.1%** 上限） |

- **精度路径定死**：打分 fp16 张量核 + fp32 累加；PV 把概率拆成 `p_hi + p_lo` 两个 fp16。
  **不许**把 p 直接舍成 fp16（实测 70 ULP），**不用** `ieee` fp32 点积（慢 4.6x）。
- **判据改过两次**（记录在报告第二节与 D42）：① "≥99% 逐位相同"降级为统计量（随元素数单调下降）；
  ② 补 **1e-5 绝对兜底** —— 真数据里有元素是"大数相消"出来的，fp32 归约顺序差异会被放大成几十 ULP。
- **超 2 ULP 的根因已隔离**（7 个实验）：打分点积的 **fp32 归约顺序**差异，不是核写错。
  反量化与 `torch.dequantize_int8` **逐位一致**、张量核确实是 fp32 累加、Kahan 补偿**零改善**。
- ⚠️ **工程警告**：`tl.dot(..., input_precision="ieee")` 在本核里**只在 `num_warps=4` 下正确**，
  warps=8 实测异常（未归因）。默认路径不依赖它。
- ✅ **已接进解码路径**：见下一节（**六·补十二**，D43）。

---

## 六·补十二 · 第六轮 ①.5：int8 流式 cache 接进解码路径（✅ 2026-09-24 —— **4K 整步 3.26x，KV 常驻 0.54x**）

**报告：[reports/kv-int8-streaming-cache.md](reports/kv-int8-streaming-cache.md) · 决策 D43 · 代码 `src/nova/kvattn.py`（`KVInt8Cache`）+ `src/nova/decode.py`（`quant=` 开关）·
测试 `tests/test_kvattn_stream.py`（17 条）+ `tests/test_graph_decode_int8.py`（4 条）·
基准 `src/diagnostics/bench_int8_decode.py` · 前置核查 `probe_kvattn_scalar.py`**

常驻 = **int8 + 一把尺子 + 一个 64 槽 fp16 尾部环**（`[0,Q)` 走 int8 核、`[Q,U)` 走环上 SDPA，
两段 `logsumexp` 合并）。

| 项 | 结果 |
|---|---|
| 与「先还原再算」逐元素一致（合成，L=40…320） | **≤ 4 ULP**（最大 2.44e-4 绝对） |
| 4096 上下文整步图解码（`clocks.sm 2475 MHz`，桶长 8192） | **115.68 → 35.49 ms/token = 3.26x** |
| 2048 / 1024 同上 | 69.94 → 33.53（**2.09x**）/ 46.47 → 33.06（**1.41x**） |
| KV 常驻（485/960/1920 MiB） | **273 / 525 / 1035 MiB（0.539x）** |
| 峰值显存（4096，桶长 8192） | **6.68 GiB**（fp16 同条件 6.59）< D17 的 7GB |
| 贪心 token 与 fp16 路径 | **4/4 相同**（判据 ≥3/4） |
| `pytest tests -q` | **114 passed**（原 93 + 流式 17 + 端到端 4） |

- **关键设计（唯一一次改设计，理由不是性能是正确性）**：把 `used` / `prefix_groups` 放进
  **显存标量**、由核内 `tl.load` 读 —— 形状按桶长固定、数值每步能涨。
  冻结进图的常量**每 64 步就过期，而过期不是变慢、是静默算错**：刚写满的组仍被当 fp16 尾部，
  而环槽已被更新的 token 覆盖 ⇒ 那段历史凭空消失。可行性先单独核查过（`probe_kvattn_scalar.py`）。
- **尾部环掩码的下界 = 前缀长度 `_qlen`**：环里落在 `[0,_qlen)` 的 token 已在前缀里，不掩掉就被**加权两次**。
- **顺手修掉两个坑**：prefill 按组循环量化（≈4600 次小 kernel）→ 每层一次，
  4096 的 prefill **18.2s → 3.65s**；prefill 掩码按头展开（1 GiB）→ `[1,1,n,hist]` 广播，
  峰值 **7.62 → 6.68 GiB**。
- 🧪 **待实测 / 未做**：① **8K 端到端数字** —— 8K 时 fp16 暂存区与 int8 常驻各 1.2 GiB，加模型约 7.0 GiB，
  试跑进入换页；修法已定（prefill 按 **64 的整数倍**分块、暂存区只留当前块）；
  ② **真模型 K/V 的流式一致性**（D42 那套复核应在流式路径上重跑）；
  ③ **S4 记忆注入与 `quant="int8"` 不兼容**（`inject` 直接写 `key_cache`，与 int8+环布局不通用）；
  ④ 跨桶 `grow()` 在 int8 下明确抛 `NotImplementedError`。
...
| ~~**D19 是否换基座**~~ | ✅ **已定（D45，2026-09-25）** | **不换基座**：机器可判部分全过；Agent 通读 6 条后判定 1/3 有明确角色越界（rp-03 的 4bit/8bit 两档都写了买家的对白/动作），未触发 ≥2/3 重估线。角色边界与 AI 味记入 S5 蒸馏目标，见 [reports/d19-base-model-and-s5.md](reports/d19-base-model-and-s5.md) |
...
| ~~**训练平台（4060 / Kaggle / 云）**~~ | ✅ **已定（D46，2026-09-25）** | **分阶段**：本地 4060 先做 10 步训练 spike + 500-1000 条学生侧管线；教师数据先用 API 教师生成（27B 4-bit 约 15-16GB，4060 跑不了）；正式训练再上 Kaggle T4 16GB 或云 24GB。D17 只锁推理，不锁训练 |
...
> **进度（2026-09-23 会话 · 第六轮 ①）：int8 融合注意力核 ✅ 落地。**
>
> 靶子没动（D38 的 int8 + K 按 token 维分组），按 D40 的结论直接写核。结构：**按 KV 头切**
> （GQA 不重复读）+ 分块在线 softmax + split-K 合并核；打分走 fp16 张量核，PV 把概率拆成
> `p_hi + p_lo` 两个 fp16。
>
> | 验收项 | 结果 |
> |------|------|
> | 与「先还原再算」的偏差 | ✅ 合成 30/30 组 ≤ **1.00 ULP**；真模型 K/V 三层全部通过（≤2 ULP 或 ≤1e-5 绝对） |
> | 16K 单层解码（CUDA Graph） | ✅ **0.201 ms vs 1.513 ms = 7.53x**（带宽 = 上限 67.4%） |
> | 8192 单层 | ✅ 0.081 ms vs 0.761 ms = **9.33x**（83.1% 上限） |
> | `pytest tests -q` | ✅ **93 passed** |
>
> **判据改过两次**（报告第二节 + D42）：① "≥99% 逐位相同"降级为统计量；② 补 1e-5 绝对兜底。
> **超 2 ULP 的根因**：打分点积的 fp32 归约顺序差异（7 个隔离实验排除其它可能）。
>
> **下一步：** 把核接进解码路径 —— 需要 **int8 流式 cache**（当前组的尺子要等 64 token 写满才定，
> KIVI 式"已量化段 + fp16 residual 窗口"）与 CUDA Graph 的形状恒定。**现在还没真省显存，省的是带宽。**

---

> **进度（2026-09-25 会话 · D19 收口）：** Agent 通读 6 条英文 RP 输出，按 D19 判据判定 **不换基座**（**D45**）。
> 6/6 英文流畅、第三人称过去时、无中文/拒答/复读循环；**rp-03（4bit+8bit）越界写买家对白/动作**，
> 是唯一明确不可用项，未触发 ≥2/3 重估线。**S5 蒸馏新增两个目标**：角色边界、反 AI 味模板。
> **下一步：写本地 4060 的 10 步训练 spike 验收清单与代码**（D46；训练平台已定：本地先跑通，正式训练再上 Kaggle/云）。

---

> **进度（2026-09-25 会话 · 训练平台 D46）：** 用户确认按阶段执行：**本地 4060 先跑 10 步训练 spike
> → 500-1000 条学生侧管线 → Kaggle/云正式训练**。教师数据先用 API 教师生成（D24 v4 七层），
> 因为 27B 4-bit 教师约 15-16GB，4060 跑不了。**下一步：写 10 步 spike 的验收清单与代码。**

---

> **进度（2026-09-26 会话 · S5 API 教师 D47）：** `http://newapi.wr.wstudio.work` 的 OpenAI 兼容端点
> + `gemini-3.1-flash-lite` smoke test 通过：5/5 `finish_reason=stop`，总 token 1537，无中文，
> 非 ASCII 只有 `’` / `—`。**rp-03 的角色越界被新 system prompt 修掉**（只写 Elara，不代写买家）。
> 产物：`src/s5_gen_samples.py` + `data/s5-samples/gemini-3.1-flash-lite.jsonl`（gitignored）。
> ⚠️ 网关 HTTPS 证书域名不匹配，当前走 HTTP；key 只在可信网络使用，不要提交。
> **下一步：用这 5 条样本跑本地 4060 的 10 步训练 spike。**

---

> **进度（2026-09-26 会话 · S5 本地训练 D48）：** 10 步 spike 通过（2 warmup + 10 measured）。
> **4-bit bnb 基座反向可用，基座梯度 0**；只训 `cross_blocks` 330,362,892 参数，
> `cross_mode="predictive"` + 梯度检查点 + `PagedAdam8bit`。loss 1.9595 → 0.7702；
> step 中位 **0.889 s**，`clocks.sm` **2280-2475 MHz**；峰值 allocated **5.45 GiB** /
